...
class Hello(slicops.sliclet.Base):
    ...
    # called in thread so we can do "work" like create device
    def handle_start(self, txn):
        self.__device = slicops.device.Device("DEV_CAMERA")
        self.__acquire_thread = threading.Thread(
            target=self.__acquire_worker, daemon=True
        )
        self.__acquire_thread.start()
        self.__device.accessor("acquire").monitor(
    ...
    # monitor callback happens in an EPICS thread
